#!/usr/bin/env python3
"""Exact-source Linux controls derived from the local PR2885 review harness.

validate/bind/check-public-head/seal perform no builds or solver launches.
...
BASE = "56df7491c9027fd28c83d8b09617c2852ccd078e"
...
def manifest(path, selected):
    ...
    require(value["schema"] == "su2-pr-refresh-focused-ci.v1", "wrong manifest schema")
    ...
    require(set(value["targets"]) == {"2882", "2883", "2884"}, "unexpected PR target set")
    for pr in selected:
...
def bind(args):
    ...
    value["targets"][args.pr] = {"source_sha": args.source_sha,
...
def run_mode(source, data, evidence, target, mode, arch, build_env):
    ...
    try:
        ...
        outcome["binary_hashes_unchanged"] = binary_hashes is not None and all(sha(source / name) == value for name, value in binary_hashes.items())
...
def run(args):
    ...
    evidence.mkdir(parents=True, exist_ok=True)
    outcome = {"schema": "su2-pr-refresh-focused-result.v1", "pr": args.pr, "source_sha": target["source_sha"],
    ...
    try:
        ...
        try:
            ...
            outcome["pass"] = False
        outcome["claim_boundary"] = "Exact-source supplementary Linux normal units and registered regression comparisons; no AD/DD, no convergence or physical validation, and not upstream protected CI."
        save(evidence / "result.json", outcome)
...
def main():
    ...
    validate.add_argument("--prs", choices=["2883+2884", "2882", "2883", "2884", "all"], default="2883+2884")
    validate.add_argument("--architectures", choices=["arm64", "x86_64", "both"], default="arm64")
    ...
    binding.add_argument("--pr", choices=["2882", "2883", "2884"], required=True)
    head = subs.add_parser("check-public-head")
    for name in ("receipt", "pr", "source-sha"):
    ...
    execution.add_argument("--pr", choices=["2882", "2883", "2884"], required=True)
    ...
    if args.action == "validate":
        prs = ["2882", "2883", "2884"] if args.prs == "all" else args.prs.split("+")
    ...
    if args.action == "bind":
        bind(args)
        return 0
    if args.action == "check-public-head":
        value = json.loads(Path(args.receipt).read_text())
        require(str(value["number"]) == args.pr and value["head_sha"] == args.source_sha
                and value["head_repository"] == "babybluechips/SU2" and value["state"] == "open", "public PR head changed or closed")
        print("Public PR head matches the registered revision.")
        return 0
    if args.action == "run":
        return run(args)
